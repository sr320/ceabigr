...
Annot <- read.csv("RAnalysis/data/proteins_398_327504.csv")
...
```{bash}
multiIntersectBed -i \
RAnalysis/data/*F_R1_val_1_10x.tab > RAnalysis/data/F_meth.10x.bed
...
meth.data <- list.files(path = "RAnalysis/data/", pattern = "_R1_val_1_10x.tab_gene_AllSamps_gene.bed$", full.names=TRUE) %>%
  set_names(.) %>% 
  map_dfr(read.csv,.id="Sample.ID", header=FALSE, sep="\t", na.string="NA", stringsAsFactors = FALSE) %>% 
  dplyr::select(c(Sample.ID:V2, V4:V6,V10)) %>%
  group_by(Sample.ID)
...
for(i in 1:length(sub_meth_table$gene)){
    ...
    # fit glm position model
    fit <- glm(matrix(c(meth, unmeth), ncol=2) ~ Treatment*Sex, 
               data=sub_meth_table1, family=binomial)
