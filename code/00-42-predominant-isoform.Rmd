...
I will use `topGO` to understand if there are biological processes overrepresented in the list of genes that had a shift in the predominant isoform when compared to genes with data for more than one isoform.
...
predIsoFemdataBP #Get summary of object
```

```{r}
test.stat <- new("classicCount", testStatistic = GOFisherTest, name = "Fisher test")
resultFisher.predIsoFemdataBP <- getSigGroups(predIsoFemdataBP, test.stat)
resultFisher.predIsoFemdataBP
```

```{r}
pvalFis.predIsoFemdataBP <- score(resultFisher.predIsoFemdataBP) #Extract p-values
head(pvalFis.predIsoFemdataBP)
hist(pvalFis.predIsoFemdataBP, 50, xlab = "p-values") #Plot histogram of p-values
```

```{r}
allRes.predIsoFemdataBP <- GenTable(predIsoFemdataBP, classic = resultFisher.predIsoFemdataBP, ranksOf = "classic", orderBy = "classic", topNodes = length(pvalFis.predIsoFemdataBP))
...
```{r}
write.csv(allRes.predIsoFemdataBP, "fem-predIso-BP-FisherTestResults.csv", quote = FALSE, row.names = FALSE) #Save dataframe
```
...
```{r}
predIsoFemWideAnnot <- sigRes.predIsoFemdataBP %>%
  left_join(., y = (predIsoFemFiltAnnot %>% filter(., change == 1)), by = "GO") %>%
  dplyr::select(., gene_name, change, t_name, Predom_exp, GO, p.value, product) %>%
  separate(., col = product, into = c("product", "transcriptVar"), sep = "%2C") %>%
  dplyr::select(., -transcriptVar) %>%
  unique(.) %>%
  full_join(x = predIsoFemWide, y = ., by = "gene_name") %>%
  dplyr::select(., gene_name, change.x, change_meth, change_exp, GO, p.value, product) %>%
  dplyr::rename(., change = "change.x") #Take significant BPGO data and join with annotation information. Select columns of interest. Separate product column by "%2C" and remove transcriptVar column. Select unique rows. This removes all "transcript variant" information so each gene is listed once per significant GOterm. Full join with the wide predominant isoform information that has change in methylation data. Select relevant columns and rename change column.
head(predIsoFemWideAnnot) #Confirm manipulation
```

```{r}
femPredIsoEnrichedBPGO <- predIsoFemWideAnnot %>%
  filter(., change == 1 & is.na(GO) == FALSE) %>%
  ggplot(aes(x = change_meth)) + 
  geom_histogram() +
  labs(x = "", y = "Number of Genes", title = "A. Enriched BPGO") +
  theme_classic(base_size = 15)

femPredIsoNotEnrichedBPGO <- predIsoFemWideAnnot %>%
  filter(., change == 1 & is.na(GO) == TRUE) %>%
  ggplot(aes(x = change_meth)) + 
  geom_histogram() +
...
```{r}
femPredIsoEnrichedBPGO / femPredIsoNotEnrichedBPGO
ggsave("fem-pred-isoform-changeMeth-enrichedBPGO.pdf", height = 8.5, width = 11)
```

```{r}
femPredIsoEnrichedBPGOExp <- predIsoFemWideAnnot %>%
  filter(., change == 1 & is.na(GO) == FALSE) %>%
  ggplot(aes(x = change_meth, y = log2(change_exp + 1))) + 
  geom_point() +    
  geom_smooth(aes(x = change_meth, y = log2(change_exp + 1)), method = lm, colour = "grey20", linewidth = 2, na.rm = TRUE) +
  stat_regline_equation(aes(x = change_meth, y = log2(change_exp + 1), label = paste(..eq.label..)), formula = y ~ x, size = 5, inherit.aes = FALSE) +
  labs(x = "", y = "log FPKM", title = "A. Enriched BPGO") +
  theme_classic(base_size = 15) + theme(legend.position = "none")

femPredIsoNotEnrichedBPGOExp <- predIsoFemWideAnnot %>%
  filter(., change == 1 & is.na(GO) == TRUE) %>%
  ggplot(aes(x = change_meth, y = log2(change_exp + 1))) + 
  geom_point() +    
  geom_smooth(aes(x = change_meth, y = log2(change_exp + 1)), method = lm, colour = "grey20", linewidth = 2, na.rm = TRUE) +
  stat_regline_equation(aes(x = change_meth, y = log2(change_exp + 1), label = paste(..eq.label..)), formula = y ~ x, size = 5, inherit.aes = FALSE) +
  labs(x = "Change in Methylation (%)", y = "log FPKM", title = "B. Not Enriched BPGO") +
  theme_classic(base_size = 15) + theme(legend.position = "none")
```

```{r}
femPredIsoEnrichedBPGOExp / femPredIsoNotEnrichedBPGOExp
ggsave("fem-pred-isoform-changeMeth-changeExp-enrichedBPGO.pdf", height = 8.5, width = 11)
```

#### Number of DML in genes with changes in the predominant isoform
...
head(predIsoFemWideDML) #Confirm dataframe creation
```

```{r}
write.csv(predIsoFemWideDML, "fem-pred-isoform-DML-annotations.csv", quote = FALSE, row.names = FALSE) #Save dataframe
```
...
predIsoFemWideDML %>%
  ...
  summarise(., count = n()) %>%
  write.csv(., "fem-pred-isoform-DML-count.csv") #Group data by DML count and change, then count the number of genes with or without predominant isoform shifts with various DML counts. Save as a csv
```

DMLcount change count
0	       0	    30253		
0	       1	    2065		
1	       0	    53		
1	       1	    2		
...
```{r}
facet_labels <- c("0" = "No Shift",
                  "1" = "Shift") #Assign labels for facets
```
...
predIsoFemWide %>%
  ...
  scale_fill_manual(values = c(plotColors[5], plotColors[8]),
                    ...
                    name = "") +
  theme_classic(base_size = 15)
ggsave("fem-pred-isoform-meth-exp-distribution.pdf", height = 8.5, width = 11)
```
...
predIsoFemWideDML %>%
  ...
  scale_fill_manual(values = c(plotColors[5], plotColors[8]),
                    ...
                    name = "") +
  theme_classic(base_size = 15)
ggsave("fem-pred-isoform-DML-distribution.pdf", height = 8.5, width = 11)
```
...
if the predominant isoform shifted under low pH  ~ change gene body methylation (exposed - control) + change in gene expression (exposed - control) + log10(gene length) + number of DML.
...
```{r}
write.csv(broom::tidy(predIsoFemModel), "fem-predo-isoform-model-output.csv", quote = FALSE, row.names = FALSE) #Save model output
```
...
```{r}
predIsoMaleFiltAnnot %>%
  dplyr::select(., gene_name, GO) %>%
  unique(.) %>%
  group_by(., gene_name) %>%
  summarise(GO = paste(GO, collapse = ",")) %>%
  write.table(., "geneid2go-male_predIso.tab", quote = FALSE, sep = "\t", col.names = FALSE, row.names = FALSE)
#Take list of genes and GOterms for maleale predominant isoform data, and remove any duplicate entries (precaution). Using genes since that was the "input" for the predominant isoform analsyis. Group by transcript ID, then summarise using paste to get all GOterms for each transcript into the same row separated by ",". Save as a tab delimited file
```

```{r}
geneID2GO <- readMappings(file = "geneid2go-male_predIso.tab") #Loading the GO annotations and GeneIDs. Each line has one transcript ID and all associated GOterms
str(head(geneID2GO)) #Confirm file structure
length(geneID2GO) #9,149 genes with annotations out of 10,559 genes with data for ≥ 2 transcripts
```
...
```{r}
predIsoMaleFiltChange <- predIsoMaleFilt %>%
  filter(., change == 1) %>%
  dplyr::select(., gene_name) #Take data and retain genes where the predominant isoform shifted (change == 1). Select the gene ID column and convert to a dataframe. Save as a new object
predIsoMaleFiltChange <- predIsoMaleFiltChange$gene_name #Save column as a vector (will not work otherwise!)
```

```{r}
predIsoMaleFiltGeneList <- factor(as.integer(geneNames %in% predIsoMaleFiltChange)) #Create a factor vector to indicate genes that shifted as significant (1) and didn't shift as not significant (0)
names(predIsoMaleFiltGeneList) <- geneNames
str(predIsoMaleFiltGeneList)
```
...
```{r}
predIsoMaledataBP <- new("topGOdata", ontology = "BP", allGenes = predIsoMaleFiltGeneList,
                         annot = annFUN.gene2GO, gene2GO = geneID2GO) #Create biological process topGO object
predIsoMaledataBP #Get summary of object
```

```{r}
test.stat <- new("classicCount", testStatistic = GOFisherTest, name = "Fisher test")
resultFisher.predIsoMaledataBP <- getSigGroups(predIsoMaledataBP, test.stat)
resultFisher.predIsoMaledataBP
```

```{r}
pvalFis.predIsoMaledataBP <- score(resultFisher.predIsoMaledataBP) #Extract p-values
head(pvalFis.predIsoMaledataBP)
hist(pvalFis.predIsoMaledataBP, 50, xlab = "p-values") #Plot histogram of p-values
```

```{r}
allRes.predIsoMaledataBP <- GenTable(predIsoMaledataBP, classic = resultFisher.predIsoMaledataBP, ranksOf = "classic", orderBy = "classic", topNodes = length(pvalFis.predIsoMaledataBP))
#Create a statistical results table with statistical test results. Order by p-value (classic), and include all results (topNodes)
head(allRes.predIsoMaledataBP)
```

```{r}
write.csv(allRes.predIsoMaledataBP, "male-predIso-BP-FisherTestResults.csv", quote = FALSE, row.names = FALSE) #Save dataframe
```

#### Match enriched GOterms with general annotation information

```{r}
sigRes.predIsoMaledataBP <- allRes.predIsoMaledataBP[1:33,c(1, 6)] #Filter significantly enriched GOterms, only keep GOID and p-value
colnames(sigRes.predIsoMaledataBP) <- c("GO", "p.value") #Change column names
head(sigRes.predIsoMaledataBP)
```

```{r}
sigRes.predIsoMaledataBP %>%
  left_join(., y = (predIsoMaleFiltAnnot %>% filter(., change == 1)), by = "GO") %>%
  dplyr::select(., gene_name, change, t_name, Predom_exp, GO, p.value, product) %>%
  separate(., col = product, into = c("product", "transcriptVar"), sep = "%2C") %>%
  dplyr::select(., -transcriptVar) %>%
  unique(.) %>%
  write.csv("male-predIso-BP-EnrichedGO-geneAnnot.csv", quote = FALSE, row.names = FALSE) #Take significant BPGO data and join with annotation information. Select columns of interest. Separate product column by "%2C" and remove transcriptVar column. Select unique rows. This removes all "transcript variant" information so each gene is listed once per significant GOterm. Save as a csv file.
```
...
```{r}
predIsoMaleWideAnnot <- sigRes.predIsoMaledataBP %>%
  left_join(., y = (predIsoMaleFiltAnnot %>% filter(., change == 1)), by = "GO") %>%
  dplyr::select(., gene_name, change, t_name, Predom_exp, GO, p.value, product) %>%
  separate(., col = product, into = c("product", "transcriptVar"), sep = "%2C") %>%
  dplyr::select(., -transcriptVar) %>%
  unique(.) %>%
  full_join(x = predIsoMaleWide, y = ., by = "gene_name") %>%
  dplyr::select(., gene_name, change.x, change_meth, change_exp, GO, p.value, product) %>%
  dplyr::rename(., change = "change.x") #Take significant BPGO data and join with annotation information. Select columns of interest. Separate product column by "%2C" and remove transcriptVar column. Select unique rows. This removes all "transcript variant" information so each gene is listed once per significant GOterm. Full join with the wide predominant isoform information that has change in methylation data. Select relevant columns and rename change column.
head(predIsoMaleWideAnnot) #Confirm manipulation
```

```{r}
malePredIsoEnrichedBPGO <- predIsoMaleWideAnnot %>%
  filter(., change == 1 & is.na(GO) == FALSE) %>%
  ggplot(aes(x = change_meth)) + 
  geom_histogram() +
  labs(x = "", y = "Number of Genes", title = "A. Enriched BPGO") +
...
malePredIsoNotEnrichedBPGO <- predIsoMaleWideAnnot %>%
  filter(., change == 1 & is.na(GO) == TRUE) %>%
  ggplot(aes(x = change_meth)) + 
  geom_histogram() +
  labs(x = "Change in Methylation (%)", y = "Number of Genes", title = "B. Not Enriched BPGO") +
...
```{r}
malePredIsoEnrichedBPGO / malePredIsoNotEnrichedBPGO
ggsave("male-pred-isoform-changeMeth-enrichedBPGO.pdf", height = 8.5, width = 11)
```

```{r}
malePredIsoEnrichedBPGOExp <- predIsoMaleWideAnnot %>%
  filter(., change == 1 & is.na(GO) == FALSE) %>%
  ggplot(aes(x = change_meth, y = log2(change_exp + 1))) + 
  geom_point() +    
  geom_smooth(aes(x = change_meth, y = log2(change_exp + 1)), method = lm, colour = "grey20", linewidth = 2, na.rm = TRUE) +
...
malePredIsoNotEnrichedBPGOExp <- predIsoMaleWideAnnot %>%
  filter(., change == 1 & is.na(GO) == TRUE) %>%
  ggplot(aes(x = change_meth, y = log2(change_exp + 1))) + 
  geom_point() +    
  geom_smooth(aes(x = change_meth, y = log2(change_exp + 1)), method = lm, colour = "grey20", linewidth = 2, na.rm = TRUE) +
...
```{r}
malePredIsoEnrichedBPGOExp / malePredIsoNotEnrichedBPGOExp
ggsave("male-pred-isoform-changeMeth-changeExp-enrichedBPGO.pdf", height = 8.5, width = 11)
```

#### Number of DML in genes with changes in the predominant isoform

```{r}
predIsoMaleWideDML <- left_join(predIsoMaleWide, DMLLocationsMale, by = "gene_name") %>%
  dplyr::select(., -c(chr, gene.start, gene.end)) %>%
  group_by(., gene_name) %>%
  mutate(., DMLcount = n()) %>%
  mutate(., DMLcount = case_when(is.na(meth.diff) == TRUE ~ 0,
                                 is.na(meth.diff) == FALSE ~ DMLcount)) #Join pred iso data with DML annotations and remove superfluous columns. Group by gene name and count the number of DML per gene. When there is no DML in the gene (meth.diff == NA), change the DML count to 0.
head(predIsoMaleWideDML) #Confirm dataframe creation
```

```{r}
write.csv(predIsoMaleWideDML, "male-pred-isoform-DML-annotations.csv", quote = FALSE, row.names = FALSE) #Save dataframe
```
...
predIsoMaleWideDML %>%
  group_by(., DMLcount, change) %>%
  summarise(., count = n()) %>% 
  write.csv(., "male-pred-isoform-DML-count.csv") #Group data by DML count and change, then count the number of genes with or without predominant isoform shifts with various DML counts. Save as a csv
```
...
predIsoMaleWide %>%
  ...
  scale_fill_manual(values = c(plotColors[5], plotColors[8]),
                    ...
                    name = "") +
  theme_classic(base_size = 15)
ggsave("male-pred-isoform-meth-exp-distribution.pdf", height = 8.5, width = 11)
```
...
predIsoMaleWideDML %>%
  ...
  scale_fill_manual(values = c(plotColors[5], plotColors[8]),
                    ...
                    name = "") +
  theme_classic(base_size = 15)
ggsave("male-pred-isoform-DML-distribution.pdf", height = 8.5, width = 11)
```

#### Model data

```{r}
predIsoMaleModel <- glm(I(change == 1) ~ change_meth + DMLcount +
                          log10(length) + log2(change_exp + 1),
                        family = binomial(),
                        data = (predIsoMaleWideDML %>% dplyr::select(., -c(start, end, meth.diff)) %>% unique(.) %>% ungroup(.))) #Create a binomial model, where 1 = the predominant isoform shifted when comparing low and control pH conditions. Use change in methylation, gene length, and change in gene expression as explanatory variables
...
## Predominant isoform vs. change in methylation distribution
