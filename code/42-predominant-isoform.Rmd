...
transNoiseFem <- lm(data = femMethExpCV,
                    log10(CoV_FPKM) ~ avg_meth + log10(CoV_Meth) + poly(log2(avg_exp + 1), 2) + log10(length) + as.factor(treatment) + as.factor(treatment)*avg_meth + as.factor(treatment)*poly(log2(avg_exp + 1), 2)) #Model with quadratic relationship between expression and transcriptional noise
...
```{r}
write.csv(broom::tidy(transNoiseFem), "transcriptional-noise/fem-model-output.csv", quote = FALSE) #Save model output
```
...
femTreatmentPlot <- femMethExpCV %>%
  ...
  scale_alpha_continuous(guide = "none") +
  geom_boxplot(width = 0.1, colour = "grey20", alpha = 0.2) +
  labs(x = "Treatment", y = "Transcriptional Noise", title = expression(paste("A.   ", beta, " = -0.10, t = -43.75, p = 0"))) +
  stat_summary(aes(group = 1), fun = median, geom = "line", col = "black", lty = 2) +
...
femFPKMPlot <- femMethExpCV %>%
  ...
  scale_colour_manual(values = c(plotColors[4], plotColors[8]),
                      guide = "none") +
  scale_alpha_continuous(guide = "none") +
  labs(x = "FPKM", y = "", title = expression(paste("B.   ", beta, " = -19.26, t = -51.29, p = 0"))) +
  geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = plotColors[7], linewidth = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = plotColors[9], linewidth = 2, na.rm = TRUE) +
  theme_classic(base_size = 15)
...
femMethPlot <- femMethExpCV %>%
  ...
  scale_colour_manual(values = c(plotColors[4], plotColors[8]),
                      guide = "none") +
  scale_alpha_continuous(guide = "none") +
  labs(x = "Methylation (%)", y = "Transcriptional Noise", title = expression(paste("C.   ", beta, " = -0.004, t = -56.32, p = 0"))) +
  geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = plotColors[7], linewidth = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = plotColors[9], linewidth = 2, na.rm = TRUE) +
  theme_classic(base_size = 15)
...
femCVMethPlot <- femMethExpCV %>%
  ...
  scale_colour_manual(values = c(plotColors[4], plotColors[8]),
                      guide = "none") +
  scale_alpha_continuous(guide = "none") +
  labs(x = "CV Methylation", y = "", title = expression(paste("D.   ", beta, " = 0.10, t = 35.44, p < 1.87 x", 10^-272))) +
  geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = plotColors[7], linewidth = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = plotColors[9], linewidth = 2, na.rm = TRUE) +
  theme_classic(base_size = 15)
```
...
transNoiseMale <- lm(data = maleMethExpCV,
                     log10(CoV_FPKM) ~ avg_meth + log10(CoV_Meth) + poly(log2(avg_exp + 1), 2) + log10(length) + as.factor(treatment) + as.factor(treatment)*avg_meth + as.factor(treatment)*poly(log2(avg_exp + 1), 2))
