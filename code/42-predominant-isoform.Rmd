...
```{r}
femMethExpCV %>%
  ggplot(.) + geom_boxplot(aes(x = treatment, y = log10(CoV_FPKM), colour = treatment)) +    
  labs(x = "Treatment", y = "Transcriptional Noise") +
  theme_classic()

femMethExpCV %>%
  ggplot(.) + geom_boxplot(aes(x = as.factor(change), y = log10(CoV_FPKM), colour = as.factor(change))) +    
