...
summary(transNoiseFem) #Length and meth*treatment not significant.
```

```{r}
transNoiseFem2 <- lm(data = femMethExpCV,
                    log10(CoV_FPKM) ~ avg_meth + log10(CoV_Meth) + log2(avg_exp + 1) + as.factor(treatment) + as.factor(treatment)*log2(avg_exp + 1))
summary(transNoiseFem2) #All remaining parameters significant
```

```{r}
write.csv(as.data.frame(summary(transNoiseFem2)[4]), "transcriptional-noise/fem-model-output.csv", quote = FALSE) #Save model output
```

```{r}
hist(residuals(transNoiseFem2)) #Residuals normally distributed

plot(fitted(transNoiseFem2), residuals(transNoiseFem2)) #Variance of residuals roughly consistent for all observations. Borderline heteroscedasticity but probably okay...?
...
```{r}
anova(transNoiseFem2)
```

```{r}
transNoiseFemVP <- data.frame("variable" = rownames(anova(transNoiseFem2)),
                              "SumSq" = anova(transNoiseFem2)[[2]],
...
femTreatmentPlot <- femMethExpCV %>%
  ...
  scale_alpha_continuous(guide = "none") +
  geom_boxplot(width = 0.1, colour = "grey20", alpha = 0.2) +
  labs(x = "Treatment", y = "Transcriptional Noise", title = expression(paste("A.   ", beta, " = -0.03, t = -75.05, p < ", 10^-15))) +
  stat_summary(aes(group = 1), fun = median, geom = "line", col = "black", lty = 2) +
...
femFPKMPlot <- femMethExpCV %>%
  ...
  scale_colour_manual(values = c(plotColors[4], plotColors[8]),
                      guide = "none") +
  scale_alpha_continuous(guide = "none") +
  labs(x = "FPKM", y = "", title = expression(paste("B.   ", beta, " = -0.04, t = -47.82, p < ", 10^-15))) +
  geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = plotColors[7], linewidth = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = plotColors[9], linewidth = 2, na.rm = TRUE) +
  theme_classic(base_size = 15)
...
femMethPlot <- femMethExpCV %>%
  ...
  scale_colour_manual(values = c(plotColors[4], plotColors[8]),
                      guide = "none") +
  scale_alpha_continuous(guide = "none") +
  labs(x = "Methylation (%)", y = "Transcriptional Noise", title = expression(paste("C.   ", beta, " = -0.004, t = -78.35, p < ", 10^-15))) +
  geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = plotColors[7], linewidth = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = plotColors[9], linewidth = 2, na.rm = TRUE) +
  theme_classic(base_size = 15)
...
femCVMethPlot <- femMethExpCV %>%
  ...
  scale_colour_manual(values = c(plotColors[4], plotColors[8]),
                      guide = "none") +
  scale_alpha_continuous(guide = "none") +
  labs(x = "CV Methylation", y = "", title = expression(paste("D.   ", beta, " = 0.11, t = 38.26, p < ", 10^-15))) +
  geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = plotColors[7], linewidth = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = plotColors[9], linewidth = 2, na.rm = TRUE) +
  theme_classic(base_size = 15)
```
...
```{r}
write.csv(as.data.frame(summary(transNoiseMale2)[4]), "transcriptional-noise/male-model-output.csv", quote = FALSE) #Save model output
```
...
```{r}
anova(transNoiseMale2)
```

```{r}
transNoiseMale2VP <- data.frame("variable" = rownames(anova(transNoiseMale2)),
                              "SumSq" = anova(transNoiseMale2)[[2]],
                              "partialR2" = NA) #Create dataframe with variable names and sum of squares information from ANOVA output

for (i in 1:nrow(transNoiseMale2VP)) {
  transNoiseMale2VP$partialR2[i] <- transNoiseMale2VP$SumSq[i] / sum(transNoiseMale2VP$SumSq)
} #Calculate partial r2 by dividing sum of squares for each variable by total sum of squares (including residuals)
transNoiseMale2VP #Confirm dataframe creation
```

```{r}
write.csv(transNoiseMale2VP, "transcriptional-noise/male-model-partialR.csv", quote = FALSE, row.names = FALSE)
```
...
maleMethExpCV %>%
  ...
  # geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = plotColors[7], linewidth = 2, na.rm = TRUE) +
  # geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = plotColors[9], linewidth = 2, na.rm = TRUE) +
  geom_smooth(method = lm, colour = "grey20", linewidth = 2, na.rm = TRUE) +
  theme_classic()
...
maleTreatmentPlot <- maleMethExpCV %>%
  ...
  scale_alpha_continuous(guide = "none") +
  geom_boxplot(width = 0.1, colour = "grey20", alpha = 0.2) +
  labs(x = "Treatment", y = "Transcriptional Noise", title = expression(paste("A.   ", beta, " = 0.003, t = 0.71, p = 0.48"))) +
  stat_summary(aes(group = 1), fun = median, geom = "line", col = "black", lty = 2) +
  theme_classic(base_size = 15)
...
maleFPKMPlot <- maleMethExpCV %>%
  ...
  scale_colour_manual(values = c(plotColors[4], plotColors[8]),
                      guide = "none") +
  scale_alpha_continuous(guide = "none") +
  labs(x = "FPKM", y = "", title = expression(paste("B.   ", beta, " = -0.02, t = -11.09, p < ", 10^-15))) +
  geom_smooth(method = lm, colour = "grey20", linewidth = 2, na.rm = TRUE) +
  theme_classic(base_size = 15)
...
maleMethPlot <- maleMethExpCV %>%
  ...
  scale_colour_manual(values = c(plotColors[4], plotColors[8]),
                      guide = "none") +
  scale_alpha_continuous(guide = "none") +
  labs(x = "Methylation (%)", y = "Transcriptional Noise", title = expression(paste("C.   ", beta, " = -0.001, t = -12.36, p < ", 10^-15))) +
  geom_smooth(method = lm, colour = "grey20", linewidth = 2, na.rm = TRUE) +
  theme_classic(base_size = 15)
...
maleCVMethPlot <- maleMethExpCV %>%
  ...
  scale_colour_manual(values = c(plotColors[4], plotColors[8]),
                      guide = "none") +
  scale_alpha_continuous(guide = "none") +
  labs(x = "CV Methylation", y = "", title = expression(paste("D.   ", beta, " = 0.17, t = 30.62, p < ", 10^-15))) +
  geom_smooth(method = lm, colour = "grey20", linewidth = 2, na.rm = TRUE) +
  theme_classic(base_size = 15)
```
