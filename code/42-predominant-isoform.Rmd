...
```{r}
write.csv(as.data.frame(summary(transNoiseFem)[4]), "transcriptional-noise/fem-model-output.csv", quote = FALSE, row.names = FALSE) #Save model output
```
...
femMethExpCV %>%
  ...
  labs(x = "FPKM", y = "Transcriptional Noise") +
  geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = plotColors[7], size = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = plotColors[9], size = 2, na.rm = TRUE) +
  theme_classic()
...
femTreatmentPlot <- femMethExpCV %>%
  ...
  scale_alpha_continuous(guide = FALSE) +
  geom_boxplot(width = 0.1, colour = "grey20", alpha = 0.2) +
  labs(x = "Treatment", y = "Transcriptional Noise", title = "A") +
  stat_summary(aes(group = 1), fun = median, geom = "line", col = "black", lty = 2) +
...
femMethPlot <- femMethExpCV %>%
  ...
  scale_colour_manual(values = c(plotColors[5], plotColors[8]),
                      guide = FALSE) +
  scale_alpha_continuous(guide = FALSE) +
  labs(x = "Methylation (%)", y = "Transcriptional Noise", title = "C") +
  geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = plotColors[7], size = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = plotColors[9], size = 2, na.rm = TRUE) +
  theme_classic(base_size = 15)
...
femCVMethPlot <- femMethExpCV %>%
  ...
  scale_colour_manual(values = c(plotColors[5], plotColors[8]),
                      guide = FALSE) +
  scale_alpha_continuous(guide = FALSE) +
  labs(x = "CV Methylation", y = "", title = "D") +
  geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = plotColors[7], size = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = plotColors[9], size = 2, na.rm = TRUE) +
  theme_classic(base_size = 15)
```
...
```
```{r}
write.csv(as.data.frame(summary(transNoiseMale2)[4]), "transcriptional-noise/male-model-output.csv", quote = FALSE, row.names = FALSE) #Save model output
```
...
maleMethExpCV %>%
  ...
  # geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = plotColors[7], size = 2, na.rm = TRUE) +
  # geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = plotColors[9], size = 2, na.rm = TRUE) +
  geom_smooth(method = lm, colour = "grey20", size = 2, na.rm = TRUE) +
  theme_classic()
