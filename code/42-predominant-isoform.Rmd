...
femMethExp %>%
  ...
  geom_smooth(aes(x = avg_meth, y = log(avg_exp)), method = lm, colour = "grey20", size = 2, na.rm = TRUE) +
  theme_minimal()
...
femMethExpCV %>%
  ...
  geom_smooth(aes(x = avg_meth, y = CoV_FPKM), method = lm, colour = "grey20", size = 2, na.rm = TRUE) +
  theme_classic()
...
femMethExpCV %>%
  ...
  scale_y_continuous(limits = c(0,3)) +
  geom_smooth(aes(x = avg_meth, y = CoV_Meth), method = lm, colour = "grey20", size = 2, na.rm = TRUE) +
  theme_minimal()
...
femMethExpCV %>%
  ...
  labs(x = "FPKM", y = "Transcriptional Noise") +
  geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = "dodgerblue4", size = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = "firebrick4", size = 2, na.rm = TRUE) +
  theme_classic()
...
femMethExpCV %>%
  ...
  scale_colour_manual(values = c("dodgerblue2", "firebrick2")) +
  labs(x = "FPKM", y = "Transcriptional Noise") +
  geom_smooth(data = (. %>% filter(treatment == "control")), method = lm, colour = "dodgerblue4", size = 1.5, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), method = lm, colour = "firebrick4", size = 1.5, na.rm = TRUE) +
...
maleMethExp <- inner_join(geneMethylationMale, geneExpressionMale, by = c("gene_name", "treatment")) %>%
  ...
  select(., 1:6) %>%
  mutate(bins = case_when(avg_meth < 10 ~ "0",
                          avg_meth >= 10 & avg_meth < 20 ~ "10",
                          avg_meth >= 20 & avg_meth < 30 ~ "20",
                          avg_meth >= 30 & avg_meth < 40 ~ "30",
                          avg_meth >= 40 & avg_meth < 50 ~ "40",
...
summary(transNoiseMale2) #Reran model without length and FPKM x treatment (kept treatment). All but treatment significant
```

```{r}
transNoiseMale3 <- lm(data = maleMethExpCV,
                      log10(CoV_FPKM) ~ avg_meth + log10(CoV_Meth) + log2(avg_exp + 1))
summary(transNoiseMale3) #Reran model without treatment or meth*treatment term
```
...
```{r}
hist(residuals(transNoiseMale2)) #Residuals normally distributed

plot(fitted(transNoiseMale2), residuals(transNoiseMale2)) #Variance of residuals roughly consistent for all observations
abline(h = 0, lty = 2, col = "grey")
```
...
```{r}
maleMethExpCV %>%
  ggplot(.) + geom_point(aes(x = log2(avg_exp + 1), y = log10(CoV_FPKM), colour = treatment, alpha = 0.5)) +    
  scale_colour_manual(values = c("dodgerblue2", "firebrick2")) +
  labs(x = "FPKM", y = "Transcriptional Noise") +
  geom_smooth(data = (. %>% filter(treatment == "control")), aes(x = log2(avg_exp + 1), y = log10(CoV_FPKM)), method = lm, colour = "dodgerblue4", size = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), aes(x = log2(avg_exp + 1), y = log10(CoV_FPKM)), method = lm, colour = "firebrick4", size = 2, na.rm = TRUE) +
  geom_smooth(aes(x = log2(avg_exp + 1), y = log10(CoV_FPKM)), method = lm, colour = "black", size = 2, na.rm = TRUE) +
  theme_classic()
...
```{r}
maleMethExpCV %>%
  ggplot(.) + geom_point(aes(x = avg_meth, y = log10(CoV_FPKM), colour = treatment, alpha = 0.5)) +    
  scale_colour_manual(values = c("dodgerblue2", "firebrick2")) +
  labs(x = "Methylation (%)", y = "Transcriptional Noise") +
  geom_smooth(data = (. %>% filter(treatment == "control")), aes(x = avg_meth, y = log10(CoV_FPKM)), method = lm, colour = "dodgerblue4", size = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), aes(x = avg_meth, y = log10(CoV_FPKM)), method = lm, colour = "firebrick4", size = 2, na.rm = TRUE) +
  geom_smooth(aes(x = avg_meth, y = log10(CoV_FPKM)), method = lm, colour = "black", size = 2, na.rm = TRUE) +
  theme_classic()
...
```{r}
maleMethExpCV %>%
  ggplot(.) + geom_violin(aes(x = treatment, y = log10(CoV_FPKM), fill = treatment)) +   
  scale_fill_manual(values = c("dodgerblue2", "firebrick2")) +
  geom_boxplot(aes(x = treatment, y = log10(CoV_FPKM)), width = 0.1, colour = "grey20", alpha = 0.2) +
  labs(x = "Treatment", y = "Transcriptional Noise") +
  theme_classic()
...
```{r}
maleMethExpCV %>%
  ggplot(.) + geom_violin(aes(x = as.factor(change), y = log10(CoV_FPKM), fill = as.factor(change))) +  
  scale_fill_manual(values = c("dodgerblue2", "firebrick2")) +
  geom_boxplot(aes(x = as.factor(change), y = log10(CoV_FPKM)), width = 0.1, colour = "grey20", alpha = 0.2) +
  labs(x = "Predominant Isoform Shift", y = "Transcriptional Noise") +
  theme_classic()
...
```{r}
maleMethExpCV %>%
  ggplot(.) + geom_violin(aes(x = as.factor(bins), y = log10(CoV_FPKM)), fill = "grey80", width = 1) +  
  geom_boxplot(aes(x = as.factor(bins), y = log10(CoV_FPKM)), width = 0.1, colour = "grey20", alpha = 0.2) +
  labs(x = "Methylation (%)", y = "Transcriptional Noise") +
  stat_summary(aes(x = as.factor(bins), y = log10(CoV_FPKM), group = 1), fun = median, geom = "line", col = "black", lty = 2) +
  theme_classic() + theme(legend.position = "none",
                          strip.text.x = element_text(size = 10, face = "bold"),
                          strip.background = element_rect(linewidth = 0))
