...
femMethExpCV %>%
  ...
  geom_smooth(data = (. %>% filter(treatment == "control")), aes(x = log2(exp + 1), y = log10(CoV_FPKM)), method = lm, colour = "dodgerblue4", size = 2, na.rm = TRUE) +
  geom_smooth(data = (. %>% filter(treatment == "exposed")), aes(x = log2(exp + 1), y = log10(CoV_FPKM)), method = lm, colour = "firebrick4", size = 2, na.rm = TRUE) +
  theme_classic()
