...
```{r}
maxTransFemModel3 <- glm(I(difference == 1) ~ change_meth + DMLcount +
...
summary(maxTransFemModel3) #Only length and gene expression significant
```

```{r}
write.csv(broom::tidy(maxTransFemModel3), "fem-max-trans-binomial-model-output.csv", quote = FALSE, row.names = FALSE) #Save model output
```
