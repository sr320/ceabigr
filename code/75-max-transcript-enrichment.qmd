...
geneLengths <- cov_data_l[, c("gene_ids")] %>%
  ...
  dplyr::rename(gene_names = "gene_ids") %>%
  dplyr::select(., gene_names, length) #Add gene length to CoV data and retain only gene ID and length column. Rename columns to match naming conventions in this script
...
maxTransFemModel <- glm(difference ~ change_meth + DMLcount +
                       ...
                       data = (maxTransFemWideDML %>% dplyr::select(., -c(start, end, meth.diff)) %>% unique(.) %>% ungroup(.))) #Create a binomial model, where 1 = the predominant isoform shifted when comparing low and control pH conditions. Use change in methylation, gene length, and change in gene expression as explanatory variables
summary(maxTransFemModel) #Only length and gene expression significant
```

```{r}
step(maxTransFemModel, test = "LRT")
```

```{r}
maxTransFemModel2 <- glm(I(change == 1) ~ DMLcount + log10(length) + log2(change_exp + 1),
                        family = binomial(),
                        data = (predIsoFemWideDML %>% dplyr::select(., -c(start, end, meth.diff)) %>% unique(.) %>% ungroup(.))) #Model identified by step
summary(maxTransFemModel2) #Only length and gene expression significant
```

```{r}
write.csv(broom::tidy(maxTransFemModel2), "fem-max-trans-model-output.csv", quote = FALSE, row.names = FALSE) #Save model output
```
