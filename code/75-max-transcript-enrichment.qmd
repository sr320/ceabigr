...
Cvir_genom_feats <- fread("https://gannet.fish.washington.edu/Atumefaciens/20211209_cvir_gff-to-bed/20211209_cvir_GCF_002022765.2_genes.bed", col.names = c("chr", "start", "stop", "gene_ids", "V5", "strand")) %>%
  mutate(., length = Cvir_genom_feats$stop - Cvir_genom_feats$start + 1) #Import genome feature BEDfile and create a new column for gene length
...
maxTransFemWideDMLLengths <- maxTransFemWideDML %>%
  ...
  ungroup(.) %>%
  mutate(., difference_cat = case_when(difference > 0 ~ 1,
                                        difference == 0 ~ 0,
                                        difference < 0 ~ -1)) %>%
