...
- `color_labels`: Labels for legend for data point colors.
  ...
  3. Grey: P-value <= 0.05, but q-value > 0.05.
  5. Black: P/Q-values > 0.05
  
Example usage:
`create_volcano_plot(stat_results_merged, "log2fc", "pval", "diffexpressed", "gene_label", , "Volcano Plot", c("Higher in females", "Higher in males", "Grey", "Black"))`
  
```{r}

create_volcano_plot <- function(data, log2fc_col, pval_col, diffexpressed_col, gene_label_col, title, color_labels) {
  # Filter data based on conditions
  blue_points <- data[data[[diffexpressed_col]] != "No" & data[[log2fc_col]] <= log(1) & data[[pval_col]] <= 0.05, ]
  red_points <- data[data[[diffexpressed_col]] != "No" & data[[log2fc_col]] > log(1) & data[[pval_col]] <= 0.05, ]
  grey_points <- data[data[[pval_col]] <= 0.05 & !(data$id %in% c(blue_points$id, red_points$id)), ]
  black_points <- data[!(data$id %in% c(blue_points$id, red_points$id, grey_points$id)), ]
  ...
  ggplot(data, aes_string(x = log2fc_col, y = paste0("-log10(", pval_col, ")"))) +
    ...
    scale_color_identity(guide = "legend", labels = color_labels, breaks = c("blue", "red", "grey", "black")) +
    theme_minimal() +
...
females_DET_volcano_plot <- create_volcano_plot(
  ...
  c("Differentially Expressed - Controls",
    ...
    paste0("p-value <= ", pvalue), "P/Q-values > 0.05"
    )
  )
...
ggsave("../output/18-differential-gene-expression-ballgown/figures/volcano_plot-DETs-females-CvE.png", females_DET_volcano_plot, width = 10, height = 8, units = "in", dpi = 300)
```
...
NOTE: 0 DETs identified.
```{r DEGs-male-OA}
# Load phenotype info into Ballgown
pData(bg_males) <- sample_metadata_subset_male_only

males_DET_all_stat_results = stattest(bg_males, feature='gene', meas='FPKM', covariate="TreatmentN", getFC = TRUE)
head(males_DET_all_stat_results)

# Filter DT based on set p-values and q-values
filtered_males_DET_all_stat_results <- filter(males_DET_all_stat_results, pval <= pvalue & qval <= qvalue)
head(filtered_males_DET_all_stat_results)

# Count number of DETG.
count_filtered_males_DET_all_stat_results <-  nrow(filtered_males_DET_all_stat_results)

# Print number of DEG
cat("Number of male OA DEGs with p-values and q-values <= ", pvalue, ":", count_filtered_males_DET_all_stat_results)
```