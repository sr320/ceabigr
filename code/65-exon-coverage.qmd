...
# List to store GeneID vectors where threshold is met in ALL files
threshold_met_geneids <- NULL

# Loop through each file
for (file_name in all) {
  # Construct the file path
  file_path <- paste0("../output/65-exon-coverage/", file_name, "_exon_reads.processed_data.csv")
  
  # Read the data frame
  current_data <- read.csv(file_path)
  
  # Apply the processing function with threshold
  result <- process_data_with_threshold(current_data, exon_sum_threshold)
  
  # Store the result in the list
  results_list[[file_name]] <- result
  
  # Extract GeneID where threshold is met
  current_geneids <- result$GeneID
  if (is.null(threshold_met_geneids)) {
    threshold_met_geneids <- current_geneids
  } else {
    threshold_met_geneids <- intersect(threshold_met_geneids, current_geneids)
  }
}

# Convert the list of GeneID vectors to a single vector
final_geneids <- unlist(threshold_met_geneids)
