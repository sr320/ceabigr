...
# if{} statement skips the `all` comparison
...
# Calculate mean methylation of each gene in each comparison
# Since each data frame is the same two-column format, can reference by column numbers 1 and 2
# if{} statement skips the `all` comparison
mean.gene.methylation.CoV.list <- lapply(mean.gene.methylation.CoV.list,
                                          function(df) {
                                              if (ncol(df) >= 2) { 
                                            df$mean.methylation <- rowMeans(df[,c(1,2)], na.rm = TRUE)
                                              }
                                            return(df)
                                            }
                                          )
...
Since all data frames are formatted the same (comp1, comp2, abs.delta, mean.methylation)
we can refer to them by column numbers.
...
lapply(
  ...
  function(df) {
    if (ncol(df) > 2) {
      names <- names(df)
      df %>% ggplot(aes_string(x=names[2], y=names[3], colour=names[4])) +
...
lapply(
  ...
  function(df) {
      if (ncol(df) > 2) {
      names <- names(df)
      df %>% ggplot(aes_string(x=names[2], y=names[3], colour=names[5])) +
      geom_point(alpha = 0.6, size = 3) +
      geom_smooth(method = "lm") +
      geom_abline(size=1.0,colour="red") +
      sc
