...
## Calculate mean methylation coefficients of variation (CoV) for all samples
```{r calculate-mean-methylation-coefficients-of-variation-for-all-samples}
...
for (comparison in comparisons) {
  ...
  if (comparison == "all") {
    ...
    methylation.transposed.rownames.list[[comparison]][treatment_col] <-
      as.data.frame(
        rowcvs(
    ...
    methylation.transposed.rownames.list[[comparison]][first_treatment_col] <-
      as.data.frame(
        methylation.transposed.matrix %>%
    ...
    methylation.transposed.rownames.list[[comparison]][second_treatment_col] <-
      as.data.frame(
        methylation.transposed.matrix %>%
...
methylation.transposed.rownames.list <- lapply(methylation.transposed.rownames.list, as.data.frame)

```

## Save methylation.transposed.rownames.list as R Object
...
save(methylation.transposed.rownames.list, file = "../output/40-gene-methylation/methylation.transposed.rownames.list.RData")
...
methylation.transposed.rownames.list <- lapply(methylation.transposed.rownames.list, function(x){ rownames(x) <- gene.names; x })

# Convert row names to column and assign column name
methylation.transposed.rownames.list <- lapply(methylation.transposed.rownames.list, function(x){ rownames_to_column(x, var = "gene.id") })
...
sapply(names(methylation.transposed.rownames.list),
       function(x) write.csv(methylation.transposed.rownames.list[[x]],
