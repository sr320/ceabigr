...
females <- c(controls_females, exposed_females)
males <- c(controls_males, exposed_males)

# Declare list for storing methylation dataframes
methylation.transposed.rownames.list <- list()

# Vector of treatment comparisons
comparisons <- c("all",
                 "females_v_males",
                 "controls_v_exposed",
                 "females.controls_v_females.exposed",
                 "males.controls_v_males.exposed",
                 "females.controls_v_males.controls",
                 "females.exposed_v_males.exposed"
                 )
...
```{r define-functions}
### Function to convert 
CoV.transform <- function(methylation.transposed.rownames) {
  ...
  return(methylation.transposed.matrix)
}
...
for (comparison in comparisons) {
  ...
  # Se second treatment column name
  ...
  if (comparison == "all") {
    ...
    treatment_col <- paste("All", "Mean.Methylation.CoV", sep = ".")
    ...
    methylation.transposed.rownames.list[[comparison]] <-
      as.data.frame(
        rowcvs(
    ...
    # Rename Coefficient of Variation column
    # the column is actually named: `rowcvs(methylation.transposed.matrix, unbiased = TRUE)`
    # after the rowcvs() function runs
    # Not sure what the `!! :=` notation means in the rename() function!!??
    methylation.transposed.rownames.list[[comparison]] <- 
      rename(methylation.transposed.rownames.list[[comparison]], 
             !!treatment_col := `rowcvs(methylation.transposed.matrix, unbiased = TRUE)`
             )
  }
  
  ##### Females vs males #####
  if (comparison == "females_v_males") {
    # If matrix doesn't exist, create it
    if (!exists(methylation.transposed.rownames.list[[comparison]])) {
    ...
    methylation.transposed.rownames.list[[comparison]] <- 
      rename(methylation.transposed.rownames.list[[comparison]], 
             !!first_treatment_col := `rowcvs(methylation.transposed.matrix, unbiased = TRUE)`
             )
        
      # If matrix does exist, append column to matrix
      } else
          methylation.transposed.rownames.list[[comparison]] <- cbind(methylation.transposed.rownames.list[[comparison]])
    ...
    methylation.transposed.rownames.list[[comparison]] <- 
      rename(methylation.transposed.rownames.list[[comparison]], 
             !!second_treatment_col := `rowcvs(methylation.transposed.matrix, unbiased = TRUE)`
             )
  }
}
...
# Calculate mean methylation and sort by gene
## Assign to first spot in list
methylation.transposed.rownames.list[[1]] <- methylation.transposed.matrix

## Calculate CoV of methylation
methylation.Cov.all.df <- as.data.frame(rowcvs(methylation.transposed.rownames.list[[1]], unbiased = TRUE))


head(methylation.CoV.all.df)

# Apply gene names to column
methylation.Cov.all.df <- cbind("gene.names" = gene.names, methylation.Cov.all.df)
