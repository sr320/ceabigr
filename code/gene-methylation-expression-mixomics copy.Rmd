...
```{r}
geneExpression <- read.csv("/home/shared/8TB_HDD_02/ashuff/Projects/ceabigr/output/AH_gene_coverage_gene.csv", header = TRUE, row.names = 1)
geneExpressionMod <- geneExpression %>%
  unite(., geneID, 1:3) %>%
  select(., geneID, genes, sample_id) %>%
  distinct(.) %>%
  slice(., -212243) %>%
  spread(., key = geneID, value = genes, fill = NA) %>%
  column_to_rownames(., var = "sample_id") #Create geneID column with chr-start-end, select only geneID, gene_cov, and sample_id information, filter unique rows, spread data for mixOmics formatting requirements
...
```{r}
geneMethylation <- read.delim("/home/shared/8TB_HDD_02/strigg/ceabigr/output/gene_summary_allsamples.txt", header = TRUE)
geneMethylation$sample <- gsub("_", "", geneMethylation$sample) #Remove _ from sample name
geneMethylationMod <- geneMethylation %>%
  unite(., geneID, 1:3) %>%
  select(., geneID, median_meth, sample) %>%
  distinct(.) %>%
  spread(., key = geneID, value = median_meth, fill = NA) %>%
  column_to_rownames(., var = "sample") #Create geneID column with chr-start-end, select only geneID, gene_cov, and sample_id information, filter unique rows, spread data for mixOmics formatting requirements
